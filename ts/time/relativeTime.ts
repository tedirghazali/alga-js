...
export const relative = (dateValue: Date | string | number, timeZone: string = 'utc', locale: string = 'en-US', options: any = {}, type: string = 'long') => {
  ...
  let newTimeRelative = new Intl.RelativeTimeFormat(locale, options).format(, 'second')
  if(type === 'short') {
    newTimeRelative = `${Math.round(seconds)}s`
  }
  ...
  if (Math.abs(interval) > 1) {
    ...
    if(type === 'short') {
      newTimeRelative = `${Math.round(newInterval)}y`
    }
  } else {
    ...
    if (Math.abs(interval) > 1) {
      ...
      if(type === 'short') {
        newTimeRelative = `${Math.round(newInterval)}mo`
      }
    } else {
      ...
      if (Math.abs(interval) > 1) {
        ...
        if(type === 'short') {
          newTimeRelative = `${Math.round(newInterval)}d`
        }
      } else {
        ...
        if (Math.abs(interval) > 1) {
          ...
          if(type === 'short') {
            newTimeRelative = `${Math.round(newInterval)}h`
          }
        } else {
          interval = seconds / 60;
          if (Math.abs(interval) > 1) {
            ...
            if(type === 'short') {
              newTimeRelative = `${Math.round(newInterval)}m`
            }
          }
        }
      }
